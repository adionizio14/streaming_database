...
function get_mov_info(PDO $pdo, string $id, string $genre_id) {
    ...
    $sql = "SELECT title, release_year, runtime, rating, imgSrc, movie_ID
        FROM Movies, Genres
        where movie_ID= :id and genre_ID= :genre_id;";		// Select the email and password from the customer table where the email is equal to the value of :id

    // Execute the SQL query using the pdo function and fetch the result
    $mov_info = pdo($pdo, $sql, ['id' => $id, 'genre_ID' => $genre_id])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.
    ...
    $sql = "SELECT movie_ID, genre_ID FROM Movies, Genres";
    ...
    foreach ($movie_ids as $id) {
        ...
        $Movies = get_mov_info($pdo, $id, $genre_id);
...
function get_show_info(PDO $pdo, string $id) {
    ...
    $sql = "SELECT title, release_year, rating, imgSrc
        FROM Shows, Genres
        where show_ID= :id and genre_ID= :genre_id;";		// Select the email and password from the customer table where the email is equal to the value of :id

    // Execute the SQL query using the pdo function and fetch the result
    $show_info = pdo($pdo, $sql, ['id' => $id, 'genre_ID' => $genre_id])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.
    ...
    // SQL query to retrieve all toy IDs from the database
    $sql = "SELECT Show_ID, genre_ID FROM Shows, Genres";
    ...
    foreach ($show_ids as $id) {
        // Retrieve info about the toy with the current ID from the db using provided PDO connection
        $Shows = get_show_info($pdo, $id, $genre_id);
        // Add the retrieved toy information to the array
        $shos[$id] = $Shows;
    }
                ...
                <ul class="navbar-nav mr-auto">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <a class="dropdown-item" href="browse.php?genre=1">Action</a>
                            <a class="dropdown-item" href="browse.php?genre=2">Adventure</a>
